...
import {defaultDictionary, defaultUserDict, defaultWord} from "../constants-store";
...
export const useDict = create<IDictionaryStore>((set, get) => ({
    ...
    isUserDict: false,
    changeDict: () => set({
        currentDict: get().isUserDict ? get().mainDict : get().userDict,
        isUserDict: !get().isUserDict,
    }),
